...
bool NoximGlobalTrafficTable::load(const char *fname)
{
    ...
    traffic_table.clear();
	hash_traffic_table.resize( MAX_ID + 1 );
	for( int i = 0 ; i < hash_traffic_table.size() ; i++ )
		hash_traffic_table[i].clear();
    // Cycle reading file
    while (!fin.eof()) {
	char line[512];
	...
	if (line[0] != '\0') {
		...
		if (params >= 2) {
		    ...
		    if (params >= 3 && pir >= 0 && pir <= 1)
				...
				communication.pir = NoximGlobalParams::packet_injection_rate;
			...
			hash_traffic_table[src].push_back(communication);
		}
	    }
	}
						    ...
						    vector < pair < int, double > > &dst_prob)
{
    ...
    dst_prob.clear();
	int r_ccycle;
    for (unsigned int i = 0; i < hash_traffic_table[src_id].size(); i++) {
		NoximCommunication comm = hash_traffic_table[src_id][i];
		if (comm.src == src_id) {
...
int NoximGlobalTrafficTable::occurrencesAsSource(const int src_id)
{
	if(NoximGlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED)
		return hash_traffic_table[src_id].size();
	else return 0;
	/*int count = 0;
